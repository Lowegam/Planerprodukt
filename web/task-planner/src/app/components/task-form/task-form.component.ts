...
export class TaskFormComponent {
  ...
  isEdit = false; taskId: number | null = null;

  constructor(private ts: TaskService, private router: Router, private route: ActivatedRoute) {
    const id = this.route.snapshot.paramMap.get('id');
    if (id) { this.isEdit = true; this.taskId = +id; this.ts.getTask(this.taskId).subscribe((d: any) => this.task = d); }
  }
  save() {
    const action = this.isEdit ? this.ts.updateTask(this.taskId!, this.task) : this.ts.createTask(this.task);
